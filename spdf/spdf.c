...
#define PORT 13020       // Port number for Spdf server
#define BUFFER_SIZE 1024 // Buffer size for data transfer

// Function to handle the 'ufile' command, which uploads a file from Smain server to the Spdf server
void handle_ufile_command(int client_socket, char *filename, char *dest_path)
{
    ...
    char full_path[BUFFER_SIZE]; // Buffer to hold the full file path on the Spdf server
...
// Function to handle the 'dfile' command, which downloads a file from the Spdf server to the Smain server
void handle_dfile_command(int client_socket, char *filename)
{
    ...
    // snprintf(full_path, sizeof(full_path), "~Spdf/%s", filename);
...
// Function to handle the 'rmfile' command, which deletes a file from the Spdf server
void handle_rmfile_command(int client_socket, char *filename)
{
    // Construct the full file path by replacing the Smain prefix with Spdf
    char pdf_path[256];
    snprintf(pdf_path, sizeof(pdf_path), "%s%s", "~Spdf", filename + strlen("~Smain"));
...
// Function to handle the 'dtar' command, which creates a tarball of all PDF files and sends it to the Smain server
void handle_dtar_command(int client_socket, char *file_type)
{
    ...
    // Construct the tar command to include all PDF files in the ~/Spdf directory
    snprintf(tar_command, sizeof(tar_command), "find ~/Spdf/ -name \"*.pdf\" -type f | tar -cvf pdf_files.tar -T -");
    ...
    int fd = open("pdf_files.tar", O_RDONLY);
    if (fd < 0)
    {
    ...
    close(fd);
    printf("Tar file pdf_files.tar sent to Smain server\n");
}
...
void handle_display_command(int client_socket, char *pathname)
{
    ...
    // Ensure the path starts with ~Spdf
    char full_path[BUFFER_SIZE];
    snprintf(full_path, sizeof(full_path), "~Spdf/%s", pathname + strlen("~Smain/"));

    // Find all .pdf files in the specified directory
    snprintf(buffer, sizeof(buffer), "find %s -type f -name '*.pdf'", full_path);
    ...
    // Send the list of .pdf files to the Smain server
    while (fgets(buffer, sizeof(buffer), fp) != NULL)
    {
        ...
        {
            perror("Failed to send .pdf filenames to Smain server");
            pclose(fp);
            return;
        }
